...
# Predict P/LP and mortality using the 2 coordinates ===========================

model_data <- plot_data
model_data$is_plp <- model.matrix(~Genotype, model_data)[, 2]

if (frame == "ES") {
  model_data$Z1_fixed <- -model_data$Z1
  model_data$Z2_fixed <- -model_data$Z2
  model_data$Y1_fixed <- -model_data$Y1
  model_data$Y2_fixed <- -model_data$Y2
} else {
  model_data$Z1_fixed <- model_data$Z1
  model_data$Z2_fixed <- model_data$Z2
  model_data$Y1_fixed <- model_data$Y1
...
name_plot <- list("Probability P/LP", "Mortality") %>% 
  set_names(vars_of_interest)

results_cv <- plot_tree_probs <- vector("list", length(vars_of_interest)) %>%
  set_names(vars_of_interest)

for (i in seq_along(vars_of_interest)) {
  
  res_cv <- cross_validate_gam_tree(
    X = model_data %>% select(Z1_fixed, Z2_fixed),
    y = (model_data[[vars_of_interest[i]]]) * 1,
    n_cv = 10
  )
  
  results_cv[[vars_of_interest[i]]] <- res_cv
  
  print(median(res_cv$auc))
  print(IQR(res_cv$auc))
  
  model_gam <- gam(
    as.formula(glue("{vars_of_interest[i]} ~ -1 + te(Z1_fixed, Z2_fixed)")),
    data = model_data, family = binomial()
  )
  probs_plp <- predict(model_gam, model_data, type = "response") %>%
    logit_to_probs(.)
  ...
  plot_tree_probs[[vars_of_interest[i]]] <- model_data %>%
    mutate(plp_prob = probs_plp) %>%
    ggplot(aes(x = Z1_fixed, y = Z2_fixed, color = plp_prob)) +
    geom_point(size = 2, alpha = 0.75) +
    scale_color_paletteer_c(
      "grDevices::terrain.colors",
      name = glue("Prob. {name_var[[vars_of_interest[i]]]}")
      ) +
    ggtitle(name_plot[[vars_of_interest[i]]]) +
    tree_plot_params +
...
ggsave(
  filename = file.path(plots_root, glue("plot_tree_probs_{frame}.pdf")),
  dpi = 300,
  units = "mm",
  width = 105,
  height = 105,
