...
thicknesses among the clusters identified in the previous steps.
...
frame <- "ES"  # Cardiac phase. It can be either "ED" or "ES"
...
NNmatrix <- read_csv(
  file.path(data_root, "mesh_data", frame, paste0("decim_", decim), "raw",
            "G0_A.txt")
) %>%
...
decim_verts_idx <- read_csv(
  ...
  col_names = FALSE)
decim_verts_idx <- decim_verts_idx %>%
...
model_covariates <- wallthickness %>%
  select(Cluster, age_at_scan, sex, race) %>%
  mutate(sex = as.factor(sex),
         race = as.factor(race),
         Cluster = as.factor(Cluster)) %>%
  dummy_cols(.,
             select_columns  = c("sex", "race", "Cluster"),
             remove_selected_columns = TRUE,
             remove_first_dummy = TRUE)

results <- mur(X = cbind(1, as.matrix(model_covariates)),
               Y = as.matrix(wt_mat), extract = 8)
...
Perform multiple testing corrections and save the results:
...
cbind(results, tfce_results) %>%
    ...
    write_csv(file = file.path(
      results_root, paste0("results_tfce_", frame, 
                           "_decim_", decim, "_cluster_1_vs_0.csv")))
